...
MAIN_APPEAL_TYPE = r"(?:AgR-REspe|REspe\.?|AREspEI|R-Rp|AgR-AI|ARR|AgARR|AIRR|AgRg|AgInt|Ag\.?\s*Int\.?|AG\.?REG|Ag\.?\s*Reg\.?|Agravo(?:\s+em|\s+de|\s+no|\s+na|\s+nos|\s+nas|\s+Interno|\s+Regimental|\s+de\s+Instrumento|\.?(?:\s*Reg|\s*Int)\.?)?|REsp|AREsp|A\.?\s*REsp|AgREsp|RHC|HC|H\.C\.|MS|RMS|ADI|ADC|ADPF|RE\.?|AI|RSE|RO|RR|AIRR|EDcl|EDs|ED|Apelação|APL|Reclamação|Rcl|Recl\.|RCL|R\.?Esp\.?|Rec\.\s*Esp\.?|Recurso[\s\xa0\n]+(?:em[\s\xa0\n]+|de[\s\xa0\n]+|no[\s\xa0\n]+|na[\s\xa0\n]+|nos[\s\xa0\n]+|nas[\s\xa0\n]+|Especial(?:[\s\xa0\n]+Eleitoral)?|Ordinário|Extraordinário|de[\s\xa0\n]+Revista|em[\s\xa0\n]+Habeas[\s\xa0\n]+Corpus|em[\s\xa0\n]+Mandado[\s\xa0\n]+de[\s\xa0\n]+Segurança|Eleitoral)|Suspensão[\s\xa0\n]+de[\s\xa0\n]+Liminar[\s\xa0\n]+e[\s\xa0\n]+de[\s\xa0\n]+Sentença|SLS|Embargos[\s\xa0\n]+(?:de[\s\xa0\n]+Declaração|de[\s\xa0\n]+Divergência|em|nos|nas)?|EREsp|EAREsp|AR)"
